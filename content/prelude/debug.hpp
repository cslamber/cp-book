...
tcT void print(const T& t) {
	if constexpr (experimental::is_detected_v<is_custom, T>) print(t.print_by());
	else if constexpr (experimental::is_detected_v<is_printable, T>) cerr << t;
...
#define dbg(...) ( \
	...
	debug::print(forward_as_tuple(__VA_ARGS__)), \
	cerr << endl)
